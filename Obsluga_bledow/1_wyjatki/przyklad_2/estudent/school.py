class School:
    ...
    def _split_students_to_departments(self):
        ...
        if places_in_departments < len(self.students):
            raise Exception("Za mało miejsc w klasach")
    ...
    def students(self, value):
        if len(value) < School.MAX_STUDENTS_NUMBER:
            ...
            raise Exception(
                f"W szkole jest {School.MAX_STUDENTS_NUMBER} miejsc, nie można przyjąć {len(value)} uczniów"
            )
        self.departments = self._split_students_to_departments()
